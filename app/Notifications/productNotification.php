...
class productNotification extends Notification
{
    ...
    public function __construct($slug, $product=null)
    {

        $this->slug = $slug;
        $this->product = $product;

        $notification_message = ProductMessage::where('slug', $slug)->first();
        if (!$notification_message) {
            return false;
            throw new \Exception('ProductMessage message with slug ' . $slug . ' not found');
        }
...
//            'dispute' => $this->dispute_reason,
    ...
    public function replaceNotificationVars($notifiable)
    {
        ...
        if ($this->product) {
            $product_arr = $this->product->toArray();


            foreach (array_keys($product_arr) as $key) {
                @$this->notification_message->message = str_replace(['{product_' . $key . '}'], $product_arr[$key], $this->notification_message->message);
                @$this->notification_message->sms = str_replace('{product_' . $key . '}', $product_arr[$key], $this->notification_message->sms);
                @$this->notification_message->subject = str_replace('{product_' . $key . '}', $product_arr[$key], $this->notification_message->subject);
                @$this->notification_message->action_url = str_replace('{product_' . $key . '}', $product_arr[$key], $this->notification_message->action_url);
            }
        }
